				...
				default () {
					return [
						...
						},
						{
							...
							"iconPath": "/static/tab/ele.png",
							"selectedIconPath": "/static/tab/ele-selected.png",
							"activeColor": "#2979ff"
						},
						{
						...
						},
						{
							...
							"text": "美团",
							"onReachBottomDistance": 300,
							"iconPath": "/static/tab/meituan.png",
		...
		computed: {
			...
			elColor() {
				return (index) => {
					...
					if (this.list[index].activeColor) {
						activeColor = this.list[index].activeColor;
					}
					let pagePath = this.list[index].pagePath;
					if(pagePath) {
						if(pagePath == this.pageUrl || pagePath == '/' + this.pageUrl) return activeColor;
						else return this.inactiveColor;
					} else {

						return index == this.value ? activeColor : this.inactiveColor;
					}
				}
			}
...
<style scoped lang="scss">
	...
	@import '~/assets/uview-ui/libs/css/style.components.scss';
	.u-fixed-placeholder {
		/* #ifndef APP-NVUE */
